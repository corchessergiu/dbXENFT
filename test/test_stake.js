...
describe("Test stake functionality", async function() {
    ...
    it("Only owner of DBXeNFT may stake on it", async function() {
        ...
        await expect(dbXeNFTFactory.connect(alice).stake(ethers.utils.parseEther("1"), 1))
    ...
    it("Sending value less than the required fee will fail staking attempt", async function() {
        ...
        await dbXeNFTFactory.stake(ethers.utils.parseEther("1000"), 1, {value: ethers.utils.parseEther("1")})

        await expect(dbXeNFTFactory.stake(ethers.utils.parseEther("1000"), 1,
    ...
    it("Stake 1000 DXN basic case", async function() {
        ...
        // await dbXeNFTFactory.stake(ethers.utils.parseEther("1"), 1, {value: ethers.utils.parseEther("0.001")})
        const basePow = await dbXeNFTFactory.baseDBXeNFTPower(1)
        expect(await dbXeNFTFactory.dbxenftPower(1)).to.equal(basePow.mul(BigNumber.from(2)))
    })
    ...
    it("Multiple entries and stakings coming from different addresses", async function() {
        ...
        await dbXeNFTFactoryBob.stake(ethers.utils.parseEther("100"), 3, {value: ethers.utils.parseEther("1")})
        ...
        await dbXeNFTFactoryCarol.stake(ethers.utils.parseEther("500"), 4, {value: ethers.utils.parseEther("1")})
        
        await hre.ethers.provider.send("evm_increaseTime", [60 * 60 * 24])
        await hre.ethers.provider.send("evm_mine")

        const totalEntryPow = await dbXeNFTFactory.totalEntryPowerPerCycle(0)
        console.log(totalEntryPow)
        const firstCycleRewardPow = await dbXeNFTFactory.rewardPerCycle(0)

        const deployerEntryPow = await dbXeNFTFactory.dbxenftEntryPower(1)
        await dbXeNFTFactory.claimFees(1)
        expect(await dbXeNFTFactory.baseDBXeNFTPower(1)).to.equal(deployerEntryPow.mul(firstCycleRewardPow).div(totalEntryPow))

        const aliceEntryPow = await dbXeNFTFactory.dbxenftEntryPower(2)
        await dbXeNFTFactoryAlice.claimFees(2)
        ...
        const bobEntryPow = await dbXeNFTFactory.dbxenftEntryPower(3)
        await dbXeNFTFactoryBob.claimFees(3)
        ...
        const carolEntryPow = await dbXeNFTFactory.dbxenftEntryPower(4)
        await dbXeNFTFactoryCarol.claimFees(4)
        ...
        const ePow20 = ethers.utils.parseEther("100")
        const deployerDBXENFTPow = await dbXeNFTFactory.baseDBXeNFTPower(1)
        const aliceDBXeNFTPow = aliceBasePow.add(aliceBasePow.mul(ethers.utils.parseEther("1000")).div(ePow20))
        const bobDBXeNFTPow = bobBasePow.add(bobBasePow.mul(ethers.utils.parseEther("100")).div(ePow20))
        const carolDBXeNFTPow = carolBasePow.add(carolBasePow.mul(ethers.utils.parseEther("500")).div(ePow20))
        const newRewardPow = ethers.utils.parseEther("10000").add(ethers.utils.parseEther("10000").div(BigNumber.from("100")))
        expect(await dbXeNFTFactory.summedCyclePowers(1)).to.equal(deployerDBXENFTPow
            .add(aliceDBXeNFTPow)
            .add(bobDBXeNFTPow)
            .add(carolDBXeNFTPow)
            .add(newRewardPow)
    ...
    it("Stake power should apply in the next active cycle following an inactive one", async function(){
        ...
        await xenftAlice.approve(dbXeNFTFactory.address, 10002)
        await dbXeNFTFactoryAlice.mintDBXENFT(10002, {value: ethers.utils.parseEther("1")})
        await dbXeNFTFactoryAlice.stake(ethers.utils.parseEther("1"), 2, {value: ethers.utils.parseEther("1")})
        

        await xenftBob.bulkClaimRank(100, 90)
        await xenftBob.approve(dbXeNFTFactory.address, 10003)
        await dbXeNFTFactoryBob.mintDBXENFT(10003, {value: ethers.utils.parseEther("1")})
        await dbXeNFTFactoryBob.stake(ethers.utils.parseEther("23"), 3, {value: ethers.utils.parseEther("1")})

        await xenftCarol.bulkClaimRank(2, 100)
        await xenftCarol.approve(dbXeNFTFactory.address, 10004)
        await dbXeNFTFactoryCarol.mintDBXENFT(10004, {value: ethers.utils.parseEther("1")})
        await dbXeNFTFactoryCarol.stake(ethers.utils.parseEther("7"), 4, {value: ethers.utils.parseEther("1")})
        ...
        const ePow21 = ethers.utils.parseEther("100")
        ...
        const newRewardPow = ethers.utils.parseEther("10000").add(ethers.utils.parseEther("10000").div(BigNumber.from("100")))
        expect(await dbXeNFTFactory.summedCyclePowers(2)).to.equal(aliceDBXeNFTPow
            .add(bobDBXeNFTPow)
            .add(carolDBXeNFTPow)
            .add(newRewardPow)
            .add(await dbXeNFTFactory.summedCyclePowers(0)))
    })
    ...
    //     const deployerEntryPow = await dbXeNFTFactory.dbxenftEntryPower(0)
    ...
    //     const aliceEntryPow = await dbXeNFTFactory.dbxenftEntryPower(1)
    //     await dbXeNFTFactoryAlice.claimFees(1)
    //     const aliceBasePow = await dbXeNFTFactory.baseDBXeNFTPower(1)
    //     expect(aliceBasePow).to.equal(aliceEntryPow.mul(firstCycleRewardPow).div(totalEntryPow))
        
    //     const bobEntryPow = await dbXeNFTFactory.dbxenftEntryPower(2)
    ...
    it("Multiple stakes during the same active cycle", async function() {
        ...
        await dbXeNFTFactoryAlice.mintDBXENFT(10002, {value: ethers.utils.parseEther("1")})
        await dbXeNFTFactoryAlice.stake(ethers.utils.parseEther("1000"), 2, {value: ethers.utils.parseEther("1")})
        await dbXeNFTFactoryAlice.stake(ethers.utils.parseEther("0.3"), 2, {value: ethers.utils.parseEther("1")})
        ...
        await dbXeNFTFactoryCarol.mintDBXENFT(10004, {value: ethers.utils.parseEther("1")})
        await dbXeNFTFactoryCarol.stake(ethers.utils.parseEther("500"), 4, {value: ethers.utils.parseEther("1")})
        await dbXeNFTFactoryCarol.stake(ethers.utils.parseEther("499"), 4, {value: ethers.utils.parseEther("1")})
        ...
        const newRewardPow = ethers.utils.parseEther("10000").add(ethers.utils.parseEther("10000").div(BigNumber.from("100")))
        expect(await dbXeNFTFactory.summedCyclePowers(1)).to.equal(aliceDBXeNFTPow
            .add(bobDBXeNFTPow)
            .add(carolDBXeNFTPow)
            .add(newRewardPow)
            .add(await dbXeNFTFactory.summedCyclePowers(0)))
    ...
    it("Stake during two consecutive active cycle", async function() {
        ...
        await dbXeNFTFactoryAlice.stake(ethers.utils.parseEther("144"), 2, {value: ethers.utils.parseEther("1")})
        await dbXeNFTFactoryBob.stake(ethers.utils.parseEther("789"), 3, {value: ethers.utils.parseEther("1")})
        ...
        await dbXeNFTFactoryAlice.stake(ethers.utils.parseEther("441"), 2, {value: ethers.utils.parseEther("1")})
        await dbXeNFTFactoryBob.stake(ethers.utils.parseEther("987"), 3, {value: ethers.utils.parseEther("1")})
    ...
    it("Stake during two consecutive active cycle and update stats only after both unlocks", async function() {
        await xenft.bulkClaimRank(25, 14)
        await xenft.approve(dbXeNFTFactory.address, 10001)
        await dbXeNFTFactory.mintDBXENFT(10001, {value: ethers.utils.parseEther("1")})
        ...
        const newRewardPow = ethers.utils.parseEther("10000").add(ethers.utils.parseEther("10000").div(BigNumber.from("100")))
        const lastCycleRewardPow = newRewardPow.add(newRewardPow.div(BigNumber.from("100")))
        expect(await dbXeNFTFactory.summedCyclePowers(3)).to.equal(aliceDBXeNFTPow
            .add(bobDBXeNFTPow)
            .add(lastCycleRewardPow)
            .add(newRewardPow)
            .add(await dbXeNFTFactory.summedCyclePowers(0)))
    ...
    it("Dev fee is 2.5% and is sent straight to dev address", async function() {
        await xenftAlice.bulkClaimRank(100, 100)
        ...
        const aliceTx = await dbXeNFTFactoryAlice.stake(ethers.utils.parseEther("11"), 1, {value: ethers.utils.parseEther("1")})
        ...
        let aliceStakedEvent = aliceReceipt.events.find(function(el) {
            return el.event == "Staked"
        })
        let aliceFeesPaid = aliceStakedEvent.args.devFee
    ...
    // it("Stake consecutively during inactive cycle then active cycle", async function() {
